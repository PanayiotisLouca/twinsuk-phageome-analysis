...
# -------------------------------------------------------------------------- # 
    ...
    df <- read_rds(file.path("/scratch/users/k2480753/resistome/data/Revised_phages_microbes_metabs_DATASET.rds")) %>% 
      as.data.frame() %>%
      filter(phage_alpha_div_taxonomic_richness != 0)
...
# create new dataset with just necessary variables 
df_analyse <- df %>%
  select(
    iid,
...
phen <- data.frame(
  ...
  sex = df_analyse$gm_sex
    ...
    # transposed outcome data 
dat <- t(data.frame( 
  df_analyse[ ,phage_columns]))
...
write.table(resF,
            file = file.path("/scratch/users/k2480753/resistome/Individual_phage_heritability/Output_heritibility_viral_contigs.txt"),
            col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")

write.table(res_sat, 
            file = file.path("/scratch/users/k2480753/resistome/Individual_phage_heritability/Output_heritibility_viral_contigs_CORR_SAT.txt"),
            col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")

write.table(res_mod, 
            file =  file.path("/scratch/users/k2480753/resistome/Individual_phage_heritability/Output_heritibility_viral_contigs_MOD.txt"),
            col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
