...
cor_df <- as.data.frame(as.table(cor_matrix)) %>%
  ...
  filter(Correlation > 0.4) %>% # to reduce edges 
...
path <- file.path("/Users/panayiotislouca/Documents/KCL_files/Phages_metagenome_characterisation/Analysis/Phage_species_networks/Phage_species_ggraph_network_POSITIVE_PLOT.png")

png(path, width = 16, height = 18, units = 'in', res = 300)
...
max.overlaps = 4500,
  ...
  labs(title = "Phage-Bacteria Co-abundance Network",
